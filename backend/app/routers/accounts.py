...
def list_bank_accounts(
    ...
    query = query.order_by(
        ...
        PropertyBankAccount.account_id,
        PropertyBankAccount.valid_from.desc(),
    )
...
def create_bank_account(
    ...
    try:
        ...
        raise HTTPException(
            ...
            "Für dieses SKR04-Konto existiert bereits ein Bankkonto mit überlappendem "
            "Gültigkeitszeitraum - bitte zuerst das bisherige Konto beenden (valid_to setzen).",
        ) from exc
...
def update_bank_account(
    ...
    if payload.valid_to is not None and payload.valid_to <= bank_account.valid_from:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "valid_to muss nach valid_from liegen")
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "Änderung führt zu einem überlappenden Gültigkeitszeitraum für dieses Konto.",
        ) from exc
