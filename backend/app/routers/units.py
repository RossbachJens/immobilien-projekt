...
def _get_active_unit(db: Session, unit_id: int) -> Unit:
...
def get_unit(
    ...
    return _get_active_unit(db, unit_id)
...
def update_unit(
    ...
    unit = _get_active_unit(db, unit_id)
...
# Eine Einheit kann mehrere Miteigentümer gleichzeitig haben; valid_to = NULL
# heißt "aktuell gültig". Ein Eigentümerwechsel = alte Zeile per PATCH
# schließen (valid_to setzen) + neue Zeile per POST anlegen, damit die
# Historie erhalten bleibt (relevant für die Abrechnungsspitze-Haftung, s.
# BGH-Hinweis in der Beispiel-Jahresabrechnung).
