...
export function SettlementPeriodForm({
  ...
  const [periodStart, setPeriodStart] = useState(isoDate(lastYear(), 1, 1));
  const [periodEnd, setPeriodEnd] = useState(isoDate(lastYear(), 12, 31));
  ...
  return (
    <form onSubmit={handleSubmit} className="settlement-period-form">
      ...
      <label>
        ...
        <select
          value={resolutionId}
          onChange={(e) => setResolutionId(e.target.value ? Number(e.target.value) : "")}
        >
